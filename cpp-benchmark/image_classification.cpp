...
class Predictor {
 public:
    ...
    void WarmUp(const std::string& image_file, int count);
    void MeasurePredictions(const std::string& image_file, int count);
...
void Predictor::WarmUp(const std::string& image_file, int count)
{
...
void Predictor::MeasurePredictions(const std::string& image_file, int count)
{
  NDArray image_data = LoadInputImage(image_file);
  image_data.Slice(0, 1) -= mean_image_data;
  for (int i = 0; i < count; i++) {
    ...
    executor->Forward(false);
    auto array = executor->outputs[0];
    array.WaitToRead();
    std::chrono::high_resolution_clock::time_point end = std::chrono::high_resolution_clock::now();
    std::chrono::microseconds period =
              std::chrono::duration_cast<std::chrono::microseconds>(end - begin);

    measurements.push_back(period);

    LG << "Begin = " << begin.time_since_epoch().count() << " End = "
      << end.time_since_epoch().count() << " Period = " << period.count();
  }
}
...
int main(int argc, char** argv) {
  ...
  std::string input_shape = "3 224 224";
    ...
    while (index < argc) {
        ...
        } else if (strcmp("--warmup", argv[index]) == 0) {
            index++;
            warmupIterations = (index < argc ? strtol(argv[index], NULL, 10):10);
        } else if (strcmp("--predict", argv[index]) == 0) {
            index++;
            predictionIterations = (index < argc ? strtol(argv[index], NULL, 10):100);
  ...
  try {
    ...
    predict.WarmUp(input_image, warmupIterations);
    predict.MeasurePredictions(input_image, predictionIterations);
    predict.Calculate();
